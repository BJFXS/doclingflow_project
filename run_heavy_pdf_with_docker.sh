#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TMP_INPUT_DIR="${SCRIPT_DIR}/outputs/heavy_pdf_input"
TMP_OUTPUT_DIR="${SCRIPT_DIR}/outputs/heavy_pdf_output"

rm -rf "${TMP_INPUT_DIR}" "${TMP_OUTPUT_DIR}"
mkdir -p \
  "${TMP_INPUT_DIR}/pdf_image" \
  "${TMP_INPUT_DIR}/pdf_scan" \
  "${TMP_INPUT_DIR}/pdf_long" \
  "${TMP_INPUT_DIR}/pdf_two_column"

cp "${SCRIPT_DIR}/test_docs/pdf_image/pdf_image2.pdf" "${TMP_INPUT_DIR}/pdf_image/"
cp "${SCRIPT_DIR}/test_docs/pdf_scan/scan_1.pdf" "${TMP_INPUT_DIR}/pdf_scan/"
cp "${SCRIPT_DIR}/test_docs/pdf_long/long_01.pdf" "${TMP_INPUT_DIR}/pdf_long/"
cp "${SCRIPT_DIR}/test_docs/pdf_two_column/paper_01.pdf" "${TMP_INPUT_DIR}/pdf_two_column/"

"${SCRIPT_DIR}/run_with_docker.sh" "${TMP_INPUT_DIR}" "${TMP_OUTPUT_DIR}"
