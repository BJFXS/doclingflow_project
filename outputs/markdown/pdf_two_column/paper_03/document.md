...
$$MSA(X) := Concat(SA1(X), . . . ,SAH(X))WO, (1) and each SA head SAh(X) is defined as SAh(X) := softmax(βXW Q$$

and each SA head SA h ( X ) is defined as

$$softmax(βXW QWK⊤X⊤)XW V $$

for h = 1 , . . . , H . Here, X ∈ R S × D denotes a sequence of S tokens, each represented by a D -dimensional embedding. The weight matrices W Q_h , W K_h , W V_h ∈ R D × D H correspond to the query, key, and value projections for head h , and W O ∈ R D × D is the output projection matrix. Typically, the head dimension and scaling factor are set to D H = D/H and β = 1 / √ D H .
...
$$WQWK⊤ = WV = WV ⊤ or WQ = WK = WV = ID,$$

To have an energy function, the previous work has assumed constraints such as

$$that ∥X[i,:]∥ = 1, these operators are defined token-wisely as: Π(Y )[i,:] = Y[i,:]/∥Y[i,:]∥, ProjX(Y )[i,:] =$$
...
$$the input matrix Y ∈ RS×D as: RMSNorm(Y )[i,:] = diag(γ)Π(Y )[i,:],$$
...
$$vector X[i,:] (i = 1 ··· S) is partitioned into N-dimensional vectors, referred to as the oscillators.$$
...
$$∆X(t) = Omg(osc)(X(t)) + Proj(osc) X(t)$$vector X[i,:] (i = 1 ··· S) is partitioned into N-dimensional vectors, referred to as the oscillators.$$X(t+1) = Π(osc)$$
...
$$is monotonically decreasing as dEsingle(X)/dt ≤ 0 under the condition: WV = (WQ⊤WK + WQWK⊤)/2.$$
...
Proposition 4.2. Consider the continuous-time dynamics for multi-head SA without projection: d X /dt = ∑ H_h =1 SA h ( X ) . An energy function

$$dX/dt = PH h=1 SAh(X). An energy function Emulti(X) = −$$

is monotonically decreasing as dE multi ( X ) /dt ≤ 0 under the condition

$$h = (WQ⊤$$

where U 1(2) ,h ∈ R D × D/ (2 H ) ( h ∈ [1 , H ] ) satisfies the orthogonality condition U ⊤ k,h U_k ′ ,h ′ = δ hh ′ δ kk ′ I D/ (2 H ) .
...
$$h = U1,hU⊤ 2,h, (13) where U_1(2),h ∈ RD×D/(2H) (h ∈ [1, H]) satisfies the orthogonality condition U⊤ k,hUk′,h′ = δhh′δkk′ID/(2H).$$

where J MSA ( X ) :

$$= ∂ MSA( X ) /∂ X denotes the Jacobian of MSA .$$
...
$$∆x) ≈ f(x) + J(xt)∆x with the Jacobian J := ∂f/∂x.$$
...
$$˙x = Ωx, J(x) = Ω (continuous) (16) x(t+1) = (ID + ηΩ)x(t), J(x(t)) = ID + ηΩ (discrete w/o Norm) (17) x(t+1) = Π$$˙x = Ωx, J(x) = Ω (continuous) (16) x(t+1) = (ID + ηΩ)x(t), J(x(t)) = ID + ηΩ (discrete w/o Norm) (17) x(t+1) = Π$$(ID + ηΩ)x(t) , J(x(t)) =$$(ID + ηΩ)x(t) , J(x(t)) =$$∥y∥ (discrete w/ Norm)$$
...
![Image](paper_03/document_artifacts/image_000001_efc737d3b5f05f237a130962450bbfbd4d9ee363f25ddfa87850671ddd1f7fd9.png)
...
![Image](paper_03/document_artifacts/image_000002_f634d706c3f56d69ba4703e0df8cae0f8eb5bc8715818299edd96351dc0c1403.png)
...
![Image](paper_03/document_artifacts/image_000004_3a4ec2df40d7b388d6e27977b93dd4f29dc6dd17dcb489422df67b2698e11e13.png)

We found that the Jacobian provides an interpretation. Suppose that vec(MSA( X ( t ) )) ∈ R DS is well-approximated by J_t x_t , where x_t ∈ R DS denotes vec( X ( t ) ) and J_t = ∂ MSA( X ( t ) ) /∂ X ( t ) ∈ R DS × DS . Figure 6 shows that while E pseudo significantly decreases, the cosine similarity between vec(MSA( X ( t ) )) and J_t x_t remains high throughout iterations. Under this approximation, neglecting a constant term C in ∆ X , we obtain the relation E pseudo = -x ⊤ t S_t x_t / 2 , a quadratic form involving a symmetric matrix S_t = J_t + J ⊤ t . We further expand x_t in the orthonormal basis of S_t as x_t = ∑ DS k =1 a_k t_v k_t , and the eigenvectors are sorted in descending order of their eigenvalues ( λ_1 t ≥ · · · ≥ λ DS t ). The contribution index in the figure quantifies the extent to which the

Figure 6: Test-time inference of AKOrN on the Sudoku dataset.

![Image](paper_03/document_artifacts/image_000005_b95e2240e73e583b49f632f71f96d768c07003ad4ead2f0b15fd49b850c71747.png)

state x_t is captured by the top 2% eigenvalues, specifically ∑ k ≤ 0 . 02 DS ( a_k t ) 2 / ∑ DS k =1 ( a_k t ) 2 . During inference, the proportion of the state in the top 2% eigenspace increases monotonically, eventually dominating nearly 40% . This suggests that the loop effectively performs a computation analogous to power iteration, but constrained to real space with positive eigenvalues. As a side note, in Section B.5, a more detailed analysis of the Jacobian matrix implies that this alignment to the eigenspace is predominantly determined by a certain time-independent matrix. Thus, the Jacobian provides a meaningful and promising interpretation for the observed decrease of pseudo-energy during AKOrN inference.
...
$$∂Π(Y ) ∂Y = blockdiag({∂Π(Y )[i,:] ∂Y[i,:] }S i=1).$$

For each row, we compute the gradient of the normalized vector:

$$∂Π(Y )[i,:] ∂Y[i,:] = ∂Y[i,:]/∥Y[i,:]∥ ∂Y[i,:] (24) = 1 ∥Y[i,:]∥ ∂Y[i,:] ∂Y[i,:] + Y[i,:] ∂1/∥Y[i,:]∥ ∂Y[i,:] (25) = 1 ∥Y[i,:]∥ID − Y[i,:]$$∂Π(Y ) ∂Y = blockdiag$$∥Y[i,:]∥3 (26) = 1 ∥Y[i,:]∥(ID − Y[i,:]Y ⊤ [i,:] ∥Y[i,:]∥2 ).$$∥Y[i,:]∥(ID − Y[i,:]Y ⊤ [i,:] ∥Y[i,:]∥2 )}S i=1$$∥Y[i,:]∥ diag(γ)(ID − Y[i,:]Y ⊤ [i,:] ∥Y[i,:]∥2 )}S i=1$$∂Π(Y ) ∂Y = blockdiag({∂Π(Y )[i,:] ∂Y[i,:] }S i=1).$$RMSNorm(Y )[i,:] = diag(γ)Π(Y )[i,:],$$
...
$$(in the form of a Lipschitz constant) of the Jacobian of SA defined as JMSA(X) := ∂ MSA(X)/∂X.$$

is monotonically decreasing as dE single ( X ) /dt ≤ 0 under the condition:

$$Proof. Let ∆ = softmax( βXW QWK⊤X⊤)XW V and let A = WQWK⊤. The first-order derivative of Esingle(X) with respect to X[i,:] is: dEsingle(X) dX[i,:] (32) = −$$

Proof. Let ∆ = softmax( β XW Q W K ⊤ X ⊤ ) XW V and let A = W Q W K ⊤ . The first-order derivative of E single ( X ) with respect to X [ i, :] is:

$$Proof. Let ∆h = softmax(βXW Q$$
...
$$Zh,i∥∆h[i,:]∥2 (56) ≤ 0. (57) In the third-to-last line, we use the fact that for h ̸= h′,$$Proof. Let ∆h = softmax(βXW Q$$2,h′ = O (58) AhAh′ = U1,hU⊤$$∆h[i,:] =$$Under the given condition on the weights, A = A ⊤ we have:$$= dEmulti(X)$$= ∆⊤ h[i,:]∆h′[i,:] (61) =$$Zh,i∥∆h[i,:]∥2 (56) ≤ 0. (57) In the third-to-last line, we use the fact that for h ̸= h′,$$∥JMSA(X)∥2 ≤$$h = (WQ⊤$$where Z_i = \sum j ′ exp ( β X ⊤ [ i, :] AX [ j ′ , :] ) is the normalization term.$$

Thus, we have,

$$Generally, in continuous-time systems ˙x = f(x), the equilibrium point is Lyapunov stable if all eigenvalues λj of the Jacobian J(x) satisfy R_e(λj) ≤ 0. In Eq. (16), the Jacobian J(x) = ηΩ has$$∥y∥ (discrete w/ Norm)$$In contrast, the equilibrium points of discrete-time systems x(t+1) = f(x(t)) are Lyapunov stable if all eigenvalues λj of the Jacobian satisfy |λj| ≤1. For Eq. (17), all eigenvalues of the Jacobian J(x) = ID + ηΩ take the form 1 ± iηωj for ωj ≥ 0, implying that |λj| ≥1. Therefore, the$$Generally, in continuous-time systems ˙x = f(x), the equilibrium point is Lyapunov stable if all eigenvalues λj of the Jacobian J(x) satisfy R_e(λj) ≤ 0. In Eq. (16), the Jacobian J(x) = ηΩ has$$anti-symmetric weight matrix, i.e., Ω − γI (γ >0) [Haber and Ruthotto, 2017, Chang et al., 2019].$$In contrast, the equilibrium points of discrete-time systems x(t+1) = f(x(t)) are Lyapunov stable if all eigenvalues λj of the Jacobian satisfy |λj| ≤1. For Eq. (17), all eigenvalues of the Jacobian J(x) = ID + ηΩ take the form 1 ± iηωj for ωj ≥ 0, implying that |λj| ≥1. Therefore, the$$of Ω degenerate to the same ωj = ω. After a straightforward calculation, we obtain |λj| ≤1$$anti-symmetric weight matrix, i.e., Ω − γI (γ >0) [Haber and Ruthotto, 2017, Chang et al., 2019].$$pirically found that AKOrN approximately decreases the quantity Epseudo(t) := −T_r(X(t)⊤∆X(t)),$$C in ∆X, we obtain the relation Epseudo = −x⊤ t Stxt/2, a quadratic form involving a symmetric matrix St = Jt + J⊤$$C in ∆X, we obtain the relation Epseudo = −x⊤ t Stxt/2, a quadratic form involving a symmetric matrix St = Jt + J⊤$$t ≥ ··· ≥λDS t ). The con-$$(A⊤ + A)X[i,:] (35) = −$$
...
Proposition 4.2 is restated. Consider the continuous-time dynamics for multi-head SA without projection: d X /dt = ∑ H_h =1 SA h ( X ) . An energy function

$$(A⊤ + A)X[j,:]. (36) Under the given condition on the weights, A = A⊤we have: ∆[i,:] = (softmax(βXW V X⊤)[i,:]X(A⊤ + A))⊤/2 (37) =$$

is monotonically decreasing as dE multi ( X ) /dt ≤ 0 under the condition

$$(A⊤ + A)X[j,:]/2, (38) where Zi = P$$

where U 1(2) ,h ∈ R D × D/ (2 H ) ( h ∈ [1 , H ] ) satisfies the orthogonality condition U ⊤ k,h U_k ′ ,h ′ = δ hh ′ δ kk ′ I D/ (2 H ) .

Proof. Let ∆ h = softmax( β XW Q_h W K ⊤ h X ⊤ ) XW V_h ) and let A_h = W Q_h W K ⊤ h . The first-order derivative of E multi ( X ) with respect to X [ i, :] is, similarly to the single-head case, given by:

$$t ≥ ··· ≥λDS t ). The con-$$

Under the given condition on the weights, similar to the single-head case, we have:

$$k=1(ak t )2. During$$2,h′ = O (58) AhAh′ = U1,hU⊤$$where Z_{h,i} = \sum j ′ exp ( β X ⊤ [ i, :] A_h X [ j ′ , :] ) is the normalization term. Thus, we have,$$Proof. Let ∆ = softmax( βXW QWK⊤X⊤)XW V and let A = WQWK⊤. The first-order derivative of Esingle(X) with respect to X[i,:] is: dEsingle(X) dX[i,:] (32) = −$$= dEsingle(X)$$(A⊤ + A)X[i,:] (35) = −$$(A⊤ + A)X[j,:] · (ID − X[i,:]X⊤ [i,:])∆[i,:]$$(A⊤ + A)X[j,:]. (36) Under the given condition on the weights, A = A⊤we have: ∆[i,:] = (softmax(βXW V X⊤)[i,:]X(A⊤ + A))⊤/2 (37) =$$≤ 0, (45) where, in the last inequality, we used the fact that the matrix ID − X[i,:]X⊤ [i,:] is positive semi-$$(A⊤ + A)X[j,:]/2, (38) where Zi = P$$projection: dX/dt = PH h=1 SAh(X). An energy function Emulti(X) = −$$= dEsingle(X)$$h = (WQ⊤$$(A⊤ + A)X[j,:] · (ID − X[i,:]X⊤ [i,:])∆[i,:]$$h = U1,hU⊤ 2,h, (47) where U_1(2),h ∈ RD×D/(2H) (h ∈ [1, H]) satisfies the orthogonality condition U⊤ k,hUk′,h′ = δhh′δkk′ID/(2H).$$≤ 0, (45) where, in the last inequality, we used the fact that the matrix ID − X[i,:]X⊤ [i,:] is positive semi-$$∥a∥2 are 1 (with multiplicity D − 1) and 0 (with multiplicity 1). Hence,$$∥a∥2 are 1 (with multiplicity D − 1) and 0 (with multiplicity 1). Hence,$$∥Y[i,:]∥ diag(γ)$$

̸

$$In the third-to-last line, we use the fact that for h = h ′ ,$$∥Y[i,:]∥ diag(γ)$$Y[i,:]Y ⊤ [i,:] ∥Y[i,:]∥2$$Y[i,:]Y ⊤ [i,:] ∥Y[i,:]∥2$$Therefore, setting Y = X + η∆X = X + η (C + MSA(X)), we have$$Therefore, setting Y = X + η∆X = X + η (C + MSA(X)), we have$$R ∥ISD + ηJMSA∥2 (75) ≤ maxj(|γj|) R (1 + |η|∥JMSA(X)∥2) .$$R ∥ISD + ηJMSA∥2 (75) ≤ maxj(|γj|) R (1 + |η|∥JMSA(X)∥2) .$$J(x) =$$J(x) =$$∥y∥(ID + ηΩ), (77) satisfy |λj| ≤1, where y = (ID + ηΩ)x. We begin by computing the norm of y: ∥y∥2 = ∥(ID + ηΩ)x∥2 (78) = x⊤(ID + ηΩ⊤)(ID + ηΩ)x (79) = x⊤x + ηx⊤Ωx + ηx⊤Ω⊤x + η2x⊤Ω⊤Ωx (80) = 1 + η2∥Ωx∥2,$$∥y∥(ID + ηΩ), (77) satisfy |λj| ≤1, where y = (ID + ηΩ)x. We begin by computing the norm of y: ∥y∥2 = ∥(ID + ηΩ)x∥2 (78) = x⊤(ID + ηΩ⊤)(ID + ηΩ)x (79) = x⊤x + ηx⊤Ωx + ηx⊤Ω⊤x + η2x⊤Ω⊤Ωx (80) = 1 + η2∥Ωx∥2,$$∥ID + ηΩ∥2 = |1 ± iηω| =$$MSA(X) := Concat(SA1(X), . . . ,SAH(X))WO, (1) and each SA head SAh(X) is defined as SAh(X) := softmax(βXW Q$$∥J(x)∥2 =$$softmax(βXW QWK⊤X⊤)XW V $$∥y∥(ID + ηΩ)$$
...
$$1 + η2ω2 = 1. (87) This implies that all eigenvalues of J(x) satisfy |λj| ≤1.$$

Using Lemma A.3, we have

$$We primarily followed Miyato et al. [2025] and used their official implementation1, using N = 4 as$$WQWK⊤ = WV = WV ⊤ or WQ = WK = WV = ID,$$RE-single :=$$SA models. By defining the concatination as WV := [WV$$Therefore, setting Y = X + η ∆ X = X + η ( C +MSA( X )) , we have$$H ] ∈ RD×D, we introduce RE-multi :=$$under the condition that H = 1. If RE-single = 0, setting WV = WV$$system diverge. For a discrete-time system x(t+1) = f(x(t)), the Lyapunov spectrum{λi} is defined as: λi = lim$$system diverge. For a discrete-time system x(t+1) = f(x(t)), the Lyapunov spectrum{λi} is defined as: λi = lim$$2T log |α(T) i |, (89) where α(T)$$2T log |α(T) i |, (89) where α(T)$$Λ(T) = dfT (x(0)) dx$$Λ(T) = dfT (x(0)) dx$$λmax := max$$λmax := max$$that ∥X[i,:]∥ = 1, these operators are defined token-wisely as: Π(Y )[i,:] = Y[i,:]/∥Y[i,:]∥, ProjX(Y )[i,:] =$$λi. (91) In our experiments, we approximated the Lyapunov spectrum using a finite time horizon of^T = 16$$

## A.5 Derivation of the eigenvalue bound in oscillatory cases (Section 5.2)

We show that all eigenvalues λ j of the Jacobian

$$that models with a small number of heads (H = 1, 2) exhibit poor performance, while models with H = 8 or 16 achieve the highest accuracy.$$of Ω degenerate to the same ωj = ω. After a straightforward calculation, we obtain |λj| ≤1$$satisfy | λ_j | \le 1 , where y = ( I D + η Ω ) x .$$

We begin by computing the norm of y :

$$N ∥γ∥ maxj |γj|$$

where we used the fact that for an antisymmetric matrix Ω , x ⊤ Ω_x = 0 . Note also that an antisymmetric matrix has eigenvalues of the form ± iω j , where ω j ≥ 0 ( j = 1 , 2 , . . . ). For simplicity, assume all eigenvalues have identical magnitude ω j = ω . Then, we have

$$J = ∂ SA(X) ∂X = (IS ⊗ WV ⊤X⊤) ∂P ∂X + P ⊗ WV ⊤, (92) where P := softmax$$∥ID + ηΩ∥2 = |1 ± iηω| =$$XW QWK⊤X⊤/√DH$$∥J(x)∥2 =$$h=1 P(t)$$

Combining these, we obtain the following bound on the spectral norm of J ( x ) :

$${1 × 10−6, 5 × 10−6, . . . ,1 × 10−3} based on the test accuracy at the iteration^T = 16. We trained$$
...
We solved Sudoku task, which is a puzzle played on a_9 × 9 grid, where some of the cells are pre-filled with digits from 1 to 9 , and the remaining cells are left blank. The objective is to fill in the blank cells such that each 1) row, 2) column, and 3) 3 × 3 subgrid contains each digit exactly once.
...
$$X(t+1) = softmaxβ(X(t)WsX(t)⊤)X(t)Ws, (96) where Ws ∈ RD×D is a symmetric matrix and softmaxβ is a function reweighted with coefficient$$

under the condition that H = 1 . If R E-single = 0 , setting W V = W V_1 W O_1 satisfies the condition on W V described in Proposition 4.1.
...
The Lyapunov exponent quantifies the exponential rate at which nearby trajectories in a dynamical system diverge. For a discrete-time system x ( t +1) = f ( x ( t ) ) , the Lyapunov spectrum { λ_i } is defined as:

$$is defined as fXi,j = X[i,(j−1)N+1:jN] ∈ RN . They are defined as: eOmg(osc)(X(t))i,j = Ωj fXi,j, eProj(osc) X (Y )i,j =$$
...
![Image](paper_03/document_artifacts/image_000006_3f2d66ddfefb6e74b4e9cf4dd7c95455303d326eadc34d11fa72f744ed3b8429.png)

- (a) Number of attention heads H vs. Lyapunov exponent in ItrSA with normalization.

![Image](paper_03/document_artifacts/image_000007_5202d1e3c2fa24082ac018adc5682b9e6c5f973b0ad201ec68914dda722e6656.png)
...
$$J = ∂ SA(X) ∂X = (IS ⊗ WV ⊤X⊤) ∂P ∂X + P ⊗ WV ⊤, (92) where P := softmax$$
...
![Image](paper_03/document_artifacts/image_000008_f3c67dff220665c0b1fff85a09d37606bc54f8532dfed230fd8df33f9227d3c0.png)
...
![Image](paper_03/document_artifacts/image_000009_848d4950c2e0d33248275f8f2d72532e6d0b49df697d9f8385e301b0278eaa33.png)
...
Operation on oscillators We use ˜ X_{i,j} to refer to the j -th oscillator of the i -th token of X , which is defined as ˜ X_{i,j} = X [ i, ( j -1) N +1: jN ] ∈ R N . They are defined as:

$$is defined as fXi,j = X[i,(j−1)N+1:jN] ∈ RN . They are defined as: eOmg(osc)(X(t))i,j = Ωj fXi,j, eProj(osc) X (Y )i,j =$$

AKOrN then uses a readout module to read out patterns independent of the phase.

$$eYi,j, eΠ(osc)(Y )i,j =$$X(t+1) = softmaxβ(X(t)WsX(t)⊤)X(t)Ws, (96) where Ws ∈ RD×D is a symmetric matrix and softmaxβ is a function reweighted with coefficient$$where U kij ∈ R N ′ × N is a learned weight matrix, g is a learned function and k = 1 · · · DN .$$
