...
CONTAINER_PREFIX="${CONTAINER_PREFIX:-doclingflow2-build-check}"
STATE_FILE="${SCRIPT_DIR}/outputs/reports/.last_build_check_container"
...
docker run --name "${CONTAINER_NAME}" \
  ...
  -w /app \
  python:3.12-slim \
  bash -lc "python -m pip install --upgrade pip setuptools wheel build && python -m build"
